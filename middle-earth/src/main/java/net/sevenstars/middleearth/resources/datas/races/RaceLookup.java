...
public class RaceLookup {
    public static List<Race> getAllRaces(World world) {
        return world.getRegistryManager().getOrThrow(MiddleEarthRaces.KEY).stream().toList();
    }

    public static List<Race> getAllRaces(World world, List<Identifier> ids) {
        Registry<Race> registry = world.getRegistryManager().getOrThrow(MiddleEarthRaces.KEY);
