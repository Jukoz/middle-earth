...
public class FactionLookup {
    public static List<Faction> getAllFactions(World world) {
        return world.getRegistryManager().getOrThrow(MiddleEarthFactions.KEY).stream().toList();
    }
    public static Faction getFactionById(World world, Identifier id) throws FactionIdentifierException {
        Faction faction = world.getRegistryManager().getOrThrow(MiddleEarthFactions.KEY).get(id);
        if(faction == null)
