...
public class PointedIzherabanBlock extends Block implements Falling, Waterloggable {
    ...
    public PointedIzherabanBlock(AbstractBlock.Settings settings) {
        super(settings);
            ...
            if (isHeldByPointedDripstone(state, world, pos)) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedIzherabanBlock.DrippingFluid)optional.get()).sourceState.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
                                BlockState blockState = Blocks.CLAY.getDefaultState();
                                world.setBlockState(((PointedIzherabanBlock.DrippingFluid)optional.get()).pos, blockState);
                                Block.pushEntitiesUpBeforeBlockChange(((PointedIzherabanBlock.DrippingFluid)optional.get()).sourceState, blockState, world, ((PointedIzherabanBlock.DrippingFluid)optional.get()).pos);
                                world.emitGameEvent(GameEvent.BLOCK_CHANGE, ((PointedIzherabanBlock.DrippingFluid)optional.get()).pos, GameEvent.Emitter.of(blockState));
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos)searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedIzherabanBlock::canDrip, 11).orElse(null);
    }
    ...
    private static Optional<PointedIzherabanBlock.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
