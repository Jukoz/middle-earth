...
public class PointedLimestoneBlock extends Block implements Falling, Waterloggable {
    public static final MapCodec<PointedLimestoneBlock> CODEC = createCodec(PointedLimestoneBlock::new);
    ...
    public MapCodec<PointedLimestoneBlock> getCodec() {
        return CODEC;
    }

    public PointedLimestoneBlock(AbstractBlock.Settings settings) {
        super(settings);
            ...
            if (isHeldByPointedDripstone(state, world, pos)) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedLimestoneBlock.DrippingFluid)optional.get()).sourceState.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
                                BlockState blockState = Blocks.CLAY.getDefaultState();
                                world.setBlockState(((PointedLimestoneBlock.DrippingFluid)optional.get()).pos, blockState);
                                Block.pushEntitiesUpBeforeBlockChange(((PointedLimestoneBlock.DrippingFluid)optional.get()).sourceState, blockState, world, ((PointedLimestoneBlock.DrippingFluid)optional.get()).pos);
                                world.emitGameEvent(GameEvent.BLOCK_CHANGE, ((PointedLimestoneBlock.DrippingFluid)optional.get()).pos, GameEvent.Emitter.of(blockState));
    ...
    private static void place(WorldAccess world, BlockPos pos, Direction direction, Thickness thickness) {
        BlockState blockState = (BlockState)((BlockState)((BlockState) BlockRegistryME.POINTED_LIMESTONE.getDefaultState().with(VERTICAL_DIRECTION, direction)).with(THICKNESS, thickness)).with(WATERLOGGED, world.getFluidState(pos).getFluid() == Fluids.WATER);
    ...
    private static BlockPos getTipPos(BlockState state, WorldAccess world, BlockPos pos, int range, boolean allowMerged) {
        if (isTip(state, allowMerged)) {
            ...
            BiPredicate<BlockPos, BlockState> biPredicate = (posx, statex) -> {
                return statex.isOf(BlockRegistryME.POINTED_LIMESTONE) && statex.get(VERTICAL_DIRECTION) == direction;
            };
    ...
    private static Optional<BlockPos> getSupportingPos(World world, BlockPos pos, BlockState state, int range) {
        ...
        return searchInDirection(world, pos, direction.getOpposite().getDirection(), biPredicate, (statex) -> {
            return !statex.isOf(BlockRegistryME.POINTED_LIMESTONE);
        }, range);
    }
    ...
    private static boolean isTip(BlockState state, boolean allowMerged) {
        if (!state.isOf(BlockRegistryME.POINTED_LIMESTONE)) {
            return false;
        } else {
    ...
    private static boolean isHeldByPointedDripstone(BlockState state, WorldView world, BlockPos pos) {
        return isPointingDown(state) && !world.getBlockState(pos.up()).isOf(BlockRegistryME.POINTED_LIMESTONE);
    }
    ...
    private static boolean isPointedDripstoneFacingDirection(BlockState state, Direction direction) {
        return state.isOf(BlockRegistryME.POINTED_LIMESTONE) && state.get(VERTICAL_DIRECTION) == direction;
    }
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos)searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedLimestoneBlock::canDrip, 11).orElse(null);
    }
    ...
    private static Optional<PointedLimestoneBlock.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
    ...
    private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
        return dripstoneBlockState.isOf(StoneBlockSetRegistryME.LIMESTONE_SET.baseBlocks.base()) && waterState.isOf(Blocks.WATER) && waterState.getFluidState().isStill();
    }
