...
public class BarrowWightBrain {
    ...
    private static RandomTask<BarrowWightEntity> makeRandomWanderTask() {
    ...
    private static Optional<? extends LivingEntity> getAttackTarget(ServerWorld serverWorld, BarrowWightEntity npc) {
        return npc.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
    }

    private static boolean isTarget(ServerWorld world, BarrowWightEntity barrowWightEntity, LivingEntity target) {
        return getTarget(world, barrowWightEntity).filter(targetx -> targetx == target).isPresent();
    }

    private static Optional<? extends LivingEntity> getTarget(ServerWorld world, BarrowWightEntity barrowWightEntity) {
