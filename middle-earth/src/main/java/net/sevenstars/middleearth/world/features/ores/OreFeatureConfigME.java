...
public class OreFeatureConfigME implements FeatureConfig {
    public static final Codec<OreFeatureConfigME> CODEC = RecordCodecBuilder.create((instance) -> {
        ...
        })).apply(instance, OreFeatureConfigME::new);
    });
    ...
    public OreFeatureConfigME(List<Target> targets, BlockState blockState, int size, float discardOnAirChance) {
        this.size = size;
    ...
    public OreFeatureConfigME(RuleTest test, BlockState state, BlockState blockState, int size, float discardOnAirChance) {
        this(ImmutableList.of(new Target(test, state)), blockState, size, discardOnAirChance);
    }

    public OreFeatureConfigME(RuleTest test, BlockState state, BlockState blockState, int size) {
