...
public class OreFeature extends Feature<OreFeatureConfigME> {
    public OreFeature(Codec<OreFeatureConfigME> codec) {
        super(codec);
    }

    public boolean generate(FeatureContext<OreFeatureConfigME> context) {
        Random random = context.getRandom();
        BlockPos blockPos = context.getOrigin();
        StructureWorldAccess structureWorldAccess = context.getWorld();
        OreFeatureConfigME oreFeatureConfigME = context.getConfig();
        float f = random.nextFloat() * 3.1415927F;
        float g = (float) oreFeatureConfigME.size / 8.0F;
        int i = MathHelper.ceil(((float) oreFeatureConfigME.size / 16.0F * 2.0F + 1.0F) / 2.0F);
        ...
        for(int s = n; s <= n + q; ++s) {
            for(int t = p; t <= p + q; ++t) {
                if (o <= structureWorldAccess.getTopY(Heightmap.Type.OCEAN_FLOOR_WG, s, t)) {
                    boolean result = this.generateVeinPart(structureWorldAccess, random, oreFeatureConfigME, d, e, h, j, l, m, n, o, p, q, r);
    ...
    protected boolean generateVeinPart(StructureWorldAccess world, Random random, OreFeatureConfigME config, double startX, double endX, double startZ, double endZ, double startY, double endY, int x, int y, int z, int horizontalSize, int verticalSize) {
        int i = 0;
                                            ...
                                            if (!bitSet.get(ac)) {
                                                ...
                                                if (world.isValidForSetBlock(mutable)) {
                                                    ...
                                                    if (chunkSection != null) {
                                                        ...
                                                        while(var57.hasNext()) {
                                                            OreFeatureConfigME.Target target = (OreFeatureConfigME.Target)var57.next();
    ...
    public static boolean shouldPlace(BlockState state, Function<BlockPos, BlockState> posToState, Random random, OreFeatureConfigME config, OreFeatureConfigME.Target target, BlockPos.Mutable pos) {
