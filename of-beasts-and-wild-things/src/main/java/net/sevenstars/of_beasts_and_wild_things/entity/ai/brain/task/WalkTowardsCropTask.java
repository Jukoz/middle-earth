...
public class WalkTowardsCropTask {
    ...
    public static SingleTickTask<SnailEntity> create(float speed) {
        return create(speed, WalkTowardsCropTask::convertTargetPos, SnailEntity::isOnGround);
    }

    private static SingleTickTask<SnailEntity> create(float speed, Function<SnailEntity, Vec3d> targetGetter, Predicate<SnailEntity> shouldRun) {
    ...
    private static Vec3d convertTargetPos(SnailEntity entity) {
        ...
        return NoPenaltySolidTargeting.find((PathAwareEntity)entity, DEFAULT_HORIZONTAL_RADIUS, DEFAULT_VERTICAL_RADIUS, -2, vec3d.x, vec3d.z, 1.5707963705062866);
    }

    @Nullable
    private static BlockPos findTargetPos(SnailEntity entity) {
        ...
        if(entity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.LONG_JUMP_COOLING_DOWN).isPresent()) {
            return null;
        }

        if (entity.getWorld().getBlockState(targetPos.up()).isIn(BlockTags.CROPS)) {
            return targetPos;
        }
