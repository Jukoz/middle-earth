...
public class PointedIzherabanBlock extends Block implements LandingBlock, Waterloggable {
            ...
            if (isHeldByPointedDripstone(state, world, pos)) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedIzherabanBlock.DrippingFluid) optional.get()).blockState.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos) searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedIzherabanBlock::canDrip, 11).orElse((BlockPos) null);
    }
