...
public class PointedBlock extends Block implements LandingBlock, Waterloggable {

    public static final MapCodec<PointedBlock> CODEC = createCodec(PointedBlock::new);
    ...
    public MapCodec<PointedBlock> getCodec() {
        return CODEC;
    }

    public PointedBlock(Settings settings) {
        super(settings);
    ...
    public static void dripTick(BlockState state, ServerWorld world, BlockPos pos, float dripChance) {
        if (!(dripChance > 0.17578125F) || !(dripChance > 0.05859375F)) {
            if (isHeldByPointedDripstone(state, world, pos)) {
                Optional<PointedBlock.DrippingFluid> optional = getFluid(world, pos, state);
                if (!optional.isEmpty()) {
                    Fluid fluid = ((PointedBlock.DrippingFluid) optional.get()).fluid;
                    float f;
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedBlock.DrippingFluid) optional.get()).blockState.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos) searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedBlock::canDrip, 11).orElse((BlockPos) null);
    }
    ...
    public static Fluid getDripFluid(ServerWorld world, BlockPos pos) {
        return (Fluid) getFluid(world, pos, world.getBlockState(pos)).map((fluid) -> {
            return fluid.fluid;
        }).filter(PointedBlock::isFluidLiquid).orElse(Fluids.EMPTY);
    }

    private static Optional<PointedBlock.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
        return !isPointingDown(state) ? Optional.empty() : getSupportingPos(world, pos, state, 11).map((posx) -> {
            ...
            return new PointedBlock.DrippingFluid(blockPos, (Fluid) fluid, blockState);
        });
    }
    ...
    private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
        return dripstoneBlockState.isOf(StoneBlockSets.LIMESTONE.base()) && waterState.isOf(Blocks.WATER) && waterState.getFluidState().isStill();
    }
