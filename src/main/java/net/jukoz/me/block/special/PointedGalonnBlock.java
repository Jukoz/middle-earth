...
public class PointedGalonnBlock extends Block implements LandingBlock, Waterloggable {
    ...
    public PointedGalonnBlock(Settings settings) {
        super(settings);
            ...
            if (isHeldByPointedDripstone(state, world, pos)) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedGalonnBlock.DrippingFluid) optional.get()).blockState.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos) searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedGalonnBlock::canDrip, 11).orElse((BlockPos) null);
    }
